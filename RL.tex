...
\mat{P_a} = \begin{bmatrix} 
    ...
    \end{bmatrix}
\]
We further let the symbol $\succ$ and $\succeq$ denote strict and non-strict vectorial inequality, i.e., $x\succ y$ if and only if $\forall i \  x_i > y_i$. Recall the \linktopic{bellmanequation}{Bellman equations}
\[
\begin{split}
...
\end{split}
\]
The last term of the right hand side is the expected discounted accumulated feature value vector, or more succinctly the \phrase{feature expectations} $\mu(\pi)$ \citep{Abbeel2004Apprenticeship}.
\[
...
\]
The parameters, $\{\alpha_k\}$, are generally obtained using numerical optimization techniques to satisfy the constraints of the previous optimization problem. The normalization factor, $Z(\alpha) = \exp \big\{\sum_{k} \alpha_k f_k(x)\big\}$, forces the distribution to normalize and is also called the \phrase{partition function}.

Many of the fundamental building blocks of statistics are maximum entropy distributions. The Gaussian distribution, for example, has the form of a maximum entropy distribution constrained to \linktopic{dentropy}{match first and second moments}. 
