...
J_t(x_t) = &\ \min_{u_k} \mathbb{E}\big\{ x_t^{\top}Q_tx_t + u_t^{\top}R_tu_t + J_{t+1}(A_tx_t + B_tu_t+w_t)\big\}
\end{split}
\]
We can replace the $t$ with $N-1$, then we have 
\[
\begin{split}
J_{N-1}(x_{N-1}) =\ \min_{u_{N-1}} \mathbb{E}\big\{ & x_{N-1}^{\top}Q_{N-1}x_{N-1} + u_{N-1}^{\top}R_{N-1}u_{N-1}\\
